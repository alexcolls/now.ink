...
- Complete monitoring guide (943 lines) with:
  - Sentry error tracking integration (backend Go + mobile React Native)
  - Prometheus metrics collection with custom metrics
  - Grafana dashboards and visualization
  - Alert rules for critical metrics (error rate, latency, disk space, NFT minting)
  - Alertmanager configuration with email/Slack/PagerDuty
...
- Backend Prometheus instrumentation:
  ...
  - NFT minting metrics (total, duration, failures)
  - Video upload metrics (count, size)
  - Database query metrics
  - Active users gauge
  - Arweave upload metrics
- Alert rules for:
  - High error rate (>5%)
  - High API latency (>1s p95)
  - High database connections (>80)
  - Low disk space (<10%)
  - High NFT minting failures
  - High CPU/memory usage

### Changed
- Version bump to 0.3.3
- Production observability fully documented
